    ...
    <div className="rounded-3xl shadow-sm border border-[#e4e7ec] bg-white">
      <div className="p-6">
        ...
        <p className="text-[#475467] mt-4 text-lg">
          {producto.validity_from ? funciones.formatHour(producto.validity_from) : ""}
          {producto.validity_from && producto.validity_to ? " - " : ""}
          {producto.validity_to ? funciones.formatHour(producto.validity_to) : ""}
        </p>
        <p className="text-[#475467] mt-2 text-lg">
        {producto.value_type === "IMPORTE" && producto.value ? (
          <span>{new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS' }).format(producto.value)}</span>
        ) : null}                  </p>
      </div>
    </div>
  </div>
